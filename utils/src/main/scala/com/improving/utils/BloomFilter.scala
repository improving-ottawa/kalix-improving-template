...
object BloomFilter {
  ...
  private final val two_log2 = math.pow(2, log2)

  private final def computeBestM(capacity: Int, errorRate: Double): Int = {
    val factor = -(capacity.toDouble * math.log(errorRate)) / two_log2
    assert(factor > 0)
    factor.toInt
  }

  private final def computeBestK(n: Int, errorRate: Double): Int = {
