...
/**
  ...
  *
  * @example
  *   Use like {{{class MyTestSpecs extends IntegrationTestSpec with AnyWordSpecLike with Matchers}}}
  *
  * @note
  *   You can use this base spec with any sync specs, like `AnyWordSpecLike` or `AnyFunSpecLike`.
  * @note
  *   You __cannot__ use this base spec with any of the async specs, like `AsyncWordSpecLike`!
  *
  * @see
  *   `com.improving.testkit.IntegrationTestSpecTest` in the `integration-testkit-tests` module for an example of how to
  *   use this base spec. It should be very easy to use and "just do the right thing" (tm)
  */
abstract class IntegrationTestSpec extends internal.IntegrationTestSpecImpl {
  ...
  implicit final protected def executionContext: ExecutionContext =
    getTestKit match {
  ...
  /**
    ...
    *
    * @note
    *   It is __not__ available during class construction!
    */
  final protected def testKit: IntegrationTestKit =
    getTestKit match {
      case Some(impl) => impl
