...
object TemplateFormatter {
        ...
        case replacement :: tail =>
          findPropertyValue(lineNumber, replacement.propertyName, map) match {
            ...
            case Validated.Valid(wrongSubstitution) =>
              ...
              val errorMsg         =
                s"(line $lineNumber): value substitution expected for '${replacement.propertyName}' but $substitutionType found"
  ...
  final private def formatForeachSection(section: ForeachSection, map: SubstitutionMap): ValidatedLines = {
    ...
    val finalSectionResult =
      iteratorSubstitution.flatMap(col => formatForeachTemplateRec(col.elements.toList, List.empty, List.empty))

    Validated.fromEither(finalSectionResult)
  }

  final private def findPropertyValue(lineNo: Int, name: String, map: SubstitutionMap): Validation[Substitution] = {
    ...
    map.get(head) match {
      ...
      case None         =>
        Validated.invalid(NonEmptyChain.one(s"(line $lineNo): cannot find replacement '$name' in substitutions"))
    }
  }

  final private def combineSectionResults(results: Iterable[ValidatedLines]): ValidatedLines = {
