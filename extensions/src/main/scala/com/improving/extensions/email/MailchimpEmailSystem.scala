...
final class MailchimpEmailSystem private (private val apiKey: String, backendResource: Resource[IO, SttpBackend[IO, _]])
    extends EmailSystem {
    ...
    def logBadResponse(response: Response[Either[String, String]]): IO[Unit] = {
      response.body match {
        case Right(unexpected) =>
          safeLog(_.error(s"Got an unexpected response from Mailchimp API (/user/ping): $unexpected"))
  ...
  def sendEmail(email: Email): IO[Seq[SendResult]] = backendResource.use { backend =>
    makeRequest("/messages/send", email.toMailchimpMessage)
      ...
      .flatMap {
        ...
        case Response(Right(_), otherCode, _, _, _, _)          =>
          IO.raiseError(new RuntimeException(s"Unexpected HTTP status code: $otherCode"))
...
sealed abstract class MailchimpEmailFormatting { self: MailchimpEmailSystem.type =>
  final private def formatName(name: String): String = name.replace(" ", "")
  ...
  implicit val emailAddressEncoder: Encoder[EmailAddress] = {
    ...
    case RecipientEmailAddress(address, recipientType, recipientName) =>
      ...
      recipientName.fold(baseJson)(name => baseJson.deepMerge(Json.obj("name" -> Json.fromString(formatName(name)))))
  }
...
object MailchimpEmailSystem extends MailchimpEmailFormatting {
  final private[this] val apiEndpointBase = "https://mandrillapp.com/api/1.0"
