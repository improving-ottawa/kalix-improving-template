 ...
 *
 * @note This is not implemented for you because how OIDC sessions are stored for your specific application will
 *       depend on several factors, namely, which data storage options are available for storing the session
 *       information during "authorization token flow". <p />
 *       Some example implementations could be:
 *         - Redis
 *         - PostgreSQL Table
