...
object TemplateEngineSpec {
  ...
  private val templateText =
    """<!doctype html>
      ...
      |  </head>
      |  <body>
      |    <span class="preheader">Login Email for Example App</span>
      ...
      |                    <tr>
      |                      <td>
      |                        <p>Hi there, {{contact.firstName}} {{contact.lastName}}</p>
...
class TemplateEngineSpec extends AnyWordSpec with Matchers with ScalaFutures {
  ...
  "TemplateEngine" should {
    ...
    "be able to format a string template" in {
      ...
      val result = templateEngine.formatToStringFuture.futureValue
      result.length mustBe 10732

      result.contains(expectedString1) mustBe true
      result.contains(expectedString2) mustBe true
    }

    "be able to format a resource template" in {
      val substitutionsMap = Map(
        "contact" -> exampleAdmin
      )
      ...
      resultLines.exists(_.contains(expectedString2)) mustBe true
    }
...
//
    ...
    //  val result = templateEngine.formatLinesFuture.futureValue
    //  val templateResult = result.mkString(System.lineSeparator)
//
    //  templateResult.length must be > 0
    //  templateResult must be(expectedForeachTemplateText)
    // }

    // TODO: uncomment when using substitutions
    // "return an error when formatting with missing / invalid substitutions" in {
    //  val substitutionsMap = Map("contact" -> exampleAdmin)
    //  val templateEngine = TemplateEngine(substitutionsMap).fromTemplateString(templateText)
    //  val future = templateEngine.formatToStringFuture
    //  Await.ready(future, 1.second)
    //  val result = future.value.get
//
    //  result.isFailure mustBe true
    //  val errorMsg = result.fold(_.getMessage, _ => "")
//
    //  errorMsg.contains("cannot find replacement 'example.name'") mustBe true
    // }
